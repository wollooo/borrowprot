...
contract('TroveManager', async accounts => {
  ...
  it('redeemCollateral(): cancels the provided LUSD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    ...
    console.log("Expecte ETH: " + expectedReceivedETH)
    console.log("Recived ETH: " + receivedETH.add(toBN(GAS_PRICE * ETHFee)))
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner to claim", async () => {
    ...
    const A_GAS = toBN(th.gasUsed(await borrowerOperations.claimCollateral({ from: A, gasPrice: GAS_PRICE  })))
    ...
    const A_expectedBalance = A_balanceBefore.add(A_GAS * GAS_PRICE);
    const B_expectedBalance = B_balanceBefore - B_GAS;
    const C_expectedBalance = C_balanceBefore - C_GAS;
    ...
    const price = await priceFeed.getPrice()
    console.log("After:    " + A_balanceAfter)
    //console.log("Before:   " + A_balanceBefore)
    //console.log("Expedc:   " + A_expectedBalance)
    console.log("Expected  " + A_expectedBalance.add(A_coll.sub(A_netDebt.mul(mv._1e18BN).div(price))))

    th.assertIsApproximatelyEqual(A_balanceAfter, A_balanceBefore.sub(A_GAS).add(A_coll.sub(A_netDebt.mul(mv._1e18BN).div(price))))
