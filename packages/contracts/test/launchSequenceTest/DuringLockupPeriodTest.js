...
contract('During the initial lockup period', async accounts => {
  ...
  describe('KUMO transfer during first year after KUMO deployment', async accounts => {
    ...
    it("Kumo multisig can not transfer KUMO to a LC that was deployed directly", async () => {
      ...
      const LC_C = await LockupContract.new(kumoToken.address, A, oneYearFromSystemDeployment, { from: kumoAG })
    ...
    it("Anyone (other than kumo multisig) can transfer to an EOA", async () => {
      // Start D, E, F with some KUMO
      await kumoToken.unprotectedMint(D, dec(1, 24))
      await kumoToken.unprotectedMint(E, dec(2, 24))
      await kumoToken.unprotectedMint(F, dec(3, 24))
    ...
    it("Anyone (other than kumo multisig) can approve any EOA or to spend their KUMO", async () => {
    ...
    it("Anyone (other than kumo multisig) can increaseAllowance for any EOA or Kumo contract", async () => {
    ...
    it("Anyone (other than kumo multisig) can decreaseAllowance for any EOA or Kumo contract", async () => {
    ...
    it("Anyone (other than kumo multisig) can be the sender in a transferFrom() call", async () => {
      // Fund A, B
    ...
    it("Anyone (other than kumo AG) can stake their KUMO in the staking contract", async () => {
      // Fund F
  ...
  describe('Deploying new LCs', async accounts => {
    it("KUMO Deployer can deploy LCs through the Factory", async () => {
      ...
      const LCDeploymentTx_B = await lockupContractFactory.deployLockupContract(B, twoYearsFromSystemDeployment, { from: kumoAG })
    ...
    it("KUMO Deployer can deploy LCs directly", async () => {
      ...
      const LC_B = await LockupContract.new(kumoToken.address, B, twoYearsFromSystemDeployment, { from: kumoAG })
      const LC_B_txReceipt = await web3.eth.getTransactionReceipt(LC_B.transactionHash)

      const LC_C = await LockupContract.new(kumoToken.address, C, twoYearsFromSystemDeployment, { from: kumoAG })
    ...
    it("Anyone can deploy LCs with unlockTime = one year from deployment, directly and through factory", async () => {
      ...
      const LC_2 = await LockupContract.new(kumoToken.address, B, oneYearFromSystemDeployment, { from: kumoAG })
      ...
      const LCDeploymentTx_5 = await lockupContractFactory.deployLockupContract(C, twoYearsFromSystemDeployment, { from: kumoAG })
    ...
    describe('Withdrawal Attempts on LCs before unlockTime has passed ', async accounts => {
      ...
      it("No one can withdraw from a beneficiary's funded LC before the unlockTime", async () => {
        ...
        const variousEOAs = [teamMember_2, kumoAG, multisig, investor_1, A, C, D, E]
