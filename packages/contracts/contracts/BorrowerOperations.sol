...
contract BorrowerOperations is LiquityBase, CheckContract, IBorrowerOperations {
	...
	function openTrove(
		...
		vars.price = kumoParams.priceFeed().fetchPrice(vars.asset);
		...
		contractsCache.troveManager.setTroLiquitytus(vars.asset, msg.sender, 1);
		...
		// Move the ether to the Active Pool, and mint the LQTYmount to the borrower
		...
		_withdrawLUSD(
			...
			contractsCache.LUSDToken,
			gasPoolAddress,
			kumoParams.LUSD_GAS_COMPENSATION(vars.asset),
			kumoParams.LUSD_GAS_COMPENSATION(vars.asset)
		);
		...
		emit LUSDBorrowingFeePaid(vars.asset, msg.sender, vars.LUSDFee);
	}
	...
	function repayLUSD(
		...
		_adjustTrove(_asset, 0, msg.sender, 0, _LQTYmount, false, _upperHint, _lowerHint, 0);
	}
	...
	function _adjustTrove(
		...
		_requireNonZeroAdjustment(_collWithdrawal, _LUSDChange, _assetSent);
		_requireTroveisActive(vars.asset, contractsCache.troveManager, _borrower);

		// Confirm the operation is either a borrower adjusting their own trove, or a pure ETH transfer from the Stability Pool to a trove
		assert(
			msg.sender == _borrower ||
				(stabilityPoolManager.isStabilityPool(msg.sender) && _assetSent > 0 && _LUSDChange == 0)
		);
		...
		if (!_isDebtIncrease && _LUSDChange > 0) {
			...
			_requireValidLUSDRepayment(vars.asset, vars.debt, vars.netDebtChange);
	...
	function closeTrove(address _asset) external override {
		...
		_repayLUSD(
			...
			LUSDTokenCached,
			gasPoolAddress,
			kumoParams.LUSD_GAS_COMPENSATION(_asset)
		);
	...
	function _triggerBorrowingFee(
		...
		uint256 LUSDFee = _troveManager.getBorrowingFee(_asset, _LQTYmount);

		_requireUserAcceptsFee(LUSDFee, _LQTYmount, _maxFeePercentage);

		// Send fee to LQTY staking contract
		_LUSDToken.mint(_asset, LQTYStakingAddress, LUSDFee);
	...
	function _moveTokensAndETHfromAdjustment(
		...
		if (_isDebtIncrease) {
			_withdrawLUSD(_asset, _activePool, _LUSDToken, _borrower, _LUSDChange, _netDebtChange);
		} else {
			_repayLUSD(_asset, _activePool, _LUSDToken, _borrower, _LUSDChange);
		}
	...
	function _withdrawLUSD(
		...
		_activePool.increaseLUSDDebt(_asset, _netDebtIncrease);
		_LUSDToken.mint(_asset, _account, _LQTYmount);
	}
	...
	function _repayLUSD(
		...
		_activePool.decreaseLUSDDebt(_asset, _LUSD);
	...
	function _requireNonZeroAdjustment(
		...
		require(
			msg.value != 0 || _collWithdrawal != 0 || _LUSDChange != 0 || _assetSent != 0,
	...
	function _requireTroveisActive(
		...
		uint256 status = _troveManager.getTroLiquitytus(_asset, _borrower);
	...
	function _requireValidLUSDRepayment(
		...
		require(
			_debtRepayment <= _currentDebt.sub(kumoParams.LUSD_GAS_COMPENSATION(_asset)),
