...
contract StabilityPool is LiquityBase, CheckContract, IStabilityPool {
	...
	function provideToSP(uint256 _amount) external override {
		...
		emit FrontEndStakeChanged(newStake, msg.sender);
		...
		emit ETHGainWithdrawn(msg.sender, depositorAssetGain, LUSDLoss); // LUSD Loss required for event log
	...
	function withdrawFromSP(uint256 _amount) external override {
		...
		// Update System stake
		uint256 compoundedStake = getCompoundedTotalStake();
		uint256 newStake = compoundedStake.sub(LUSDtoWithdraw);
		_updateStakeAndSnapshots(newStake);
		emit FrontEndStakeChanged(newStake, msg.sender);
	...
	function withdrawETHGainToTrove(address _upperHint, address _lowerHint) external override {
		...
		emit FrontEndStakeChanged(compoundedSystemStake, msg.sender);
		...
		emit ETHGainWithdrawn(msg.sender, depositorAssetGain, LUSDLoss);
		emit UserDepositChanged(msg.sender, compoundedLUSDDeposit);

		assetBalance = assetBalance.sub(depositorAssetGain);
		emit StabilityPoolETHBalanceUpdated(assetBalance);
		emit EtherSent(msg.sender, depositorAssetGain);
	...
	function offset(uint256 _debtToOffset, uint256 _collToAdd) external override {
		...
		(
			...
			uint256 LUSDLossPerUnitStaked
		) = _computeRewardsPerUnitStaked(_collToAdd, _debtToOffset, totalLUSD);

		_updateRewardSumAndProduct(AssetGainPerUnitStaked, LUSDLossPerUnitStaked); // updates S and P
	...
	function _computeRewardsPerUnitStaked(
	...
	) internal returns (uint256 AssetGainPerUnitStaked, uint256 LUSDLossPerUnitStaked) {
		/*
		...
		lastAssetError_Offset = AssetNumerator.sub(AssetGainPerUnitStaked.mul(_totalLUSDDeposits));
	...
	function _moveOffsetCollAndDebt(uint256 _collToAdd, uint256 _debtToOffset) internal {
		...
		activePoolCached.decreaseLUSDDebt(assetAddress, _debtToOffset);
	...
	function _updateStakeAndSnapshots(uint256 _newValue) internal {
		...
		emit FrontEndSnapshotUpdated(currentP, currentG);
	}

	function _payOutLQTYGains(ICommunityIssuance _communityIssuance, address _depositor)
		internal
	{
	...
	function _requireUserHasTrove(address _depositor) internal view {
		require(
			troveManager.getTroLiquitytus(assetAddress, _depositor) == 1,
